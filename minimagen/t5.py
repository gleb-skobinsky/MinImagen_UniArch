...
def _check_downloads(name):
    if T5_VERSIONS[name]["tokenizer"] is None:
        T5_VERSIONS[name]["tokenizer"] = T5Tokenizer.from_pretrained(
            T5_VERSIONS[name]["handle"]
        )
    if T5_VERSIONS[name]["model"] is None:
        T5_VERSIONS[name]["model"] = T5EncoderModel.from_pretrained(
            T5_VERSIONS[name]["handle"]
        )
...
def t5_encode_text(text, name: str = "t5_base", max_length=MAX_LENGTH):
    ...
    final_encoding = final_encoding.masked_fill(
