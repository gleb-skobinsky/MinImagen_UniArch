    ...
    def __call__(self, batch):
        ...
        for elt in batch:
            ...
            if rem > 0:
                ...
                elt["encoding"] = F.pad(
                    elt["encoding"], (0, 0, 0, rem), "constant", False
                )
...
def _collate(batch):
    ...
    for didx, datum in enumerate(batch):
        for tensor in datum.keys():
            batch[didx][tensor] = batch[didx][tensor].to(
...
def _fetch_images(batch, num_threads, timeout=None, retries=0):
    fetch_single_image_with_args = partial(
        _fetch_single_image, timeout=timeout, retries=retries
    )
...
def get_minimagen_parser():
    ...
    parser.add_argument(
        ...
        "--PARAMETERS",
        dest="PARAMETERS",
        help="Parameters directory to load Imagen from",
        default=None,
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-b", "--BATCH_SIZE", dest="BATCH_SIZE", help="Batch size", default=2, type=int
    )
    parser.add_argument(
        "-mw",
        "--MAX_NUM_WORDS",
        dest="MAX_NUM_WORDS",
        help="Maximum number of words allowed in a caption",
        default=64,
        type=int,
    )
    parser.add_argument(
        ...
        "--IMG_SIDE_LEN",
        dest="IMG_SIDE_LEN",
        help="Side length of square Imagen output images",
        default=128,
        type=int,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--T5_NAME",
        dest="T5_NAME",
        help="Name of T5 encoder to use",
        default="t5_base",
        type=str,
    )
    parser.add_argument(
        "-f",
        "--TRAIN_VALID_FRAC",
        dest="TRAIN_VALID_FRAC",
        help="Fraction of dataset to use for training (vs. validation)",
        default=0.9,
        type=float,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--CHCKPT_NUM",
        dest="CHCKPT_NUM",
        help="Checkpointing batch number interval",
        default=500,
        type=int,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-rd",
        "--RESTART_DIRECTORY",
        dest="RESTART_DIRECTORY",
        help="Training directory to resume training from if restarting.",
        default=None,
        type=str,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="/content/drive/MyDrive/Colab_Notebooks/images",
    )
...
class MinimagenDataset(torch.utils.data.Dataset):
        ...
        *,
        ...
        if img_transform is None:
            ...
            self.img_transform = Compose(
                [ToTensor(), _Rescale(side_length), img_transform]
            )
    ...
    def __getitem__(self, idx):
        ...
        enc, msk = t5_encode_text(
            [self.captions[idx]], self.encoder_name, self.max_length
        )
...
def MinimagenTrain(
    ...
    def train():
        ...
        for unet_idx in range(len(unets)):
            loss = imagen(
                images, text_embeds=encoding, text_masks=mask, unet_number=unet_idx + 1
            )
        ...
        if (
            args.ACCUM_ITER == 1
            or (batch_num % args.ACCUM_ITER == 0)
            or (batch_num + 1 == len(train_dataloader))
        ):
            optimizer.step()
            optimizer.zero_grad()
        ...
        if batch_num % args.CHCKPT_NUM == 0:
            with training_dir():
                with open("training_progess.txt", "a") as f:
                    f.write(
                        f'{"-" * 10}Checkpoint created at batch number {batch_num}{"-" * 10}\n'
                    )
            ...
            with training_dir():
                with open("training_progess.txt", "a") as f:
                    f.write(
                        f"U-Nets Avg Train Losses Epoch {epoch + 1} Batch {batch_num}: "
                        f"{[round(i.item(), 3) for i in avg_loss]}\n"
                    )
                    f.write(
                        f"U-Nets Batch Train Losses Epoch {epoch + 1} Batch {batch_num}: "
    ...
    for epoch in range(args.EPOCHS):
        ...
        for batch_num, batch in tqdm(
            ...
            try:
                ...
                print("AttributeError. Timeout is not set")
                # If batch is empty, move on to the next one
                if not batch:
                    print("batch is empty")
                    continue
...
def load_restart_training_parameters(args, justparams=False):
    ...
    lines = list(
        filter(
            lambda x: True
            if True in [x.startswith(f"--{i}") for i in to_keep]
            else False,
            lines,
        )
    )
...
def save_training_info(
    args, timestamp, unets_params, imagen_params, model_size, training_dir
):
    """
